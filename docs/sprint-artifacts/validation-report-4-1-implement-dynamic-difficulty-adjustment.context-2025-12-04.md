...
**Document:** docs/sprint-artifacts/4-1-implement-dynamic-difficulty-adjustment.context.xml
...
- ✓ Story fields (asA/iWant/soThat) captured
  - **Evidence:** `<asA>game designer</asA>`, `<iWant>the AI to dynamically adjust the game's difficulty based on player performance</iWant>`, `<soThat>the challenge remains engaging and fair</soThat>`
- ✓ Acceptance criteria list matches story draft exactly (no invention)
  - **Evidence:** The `<criterion>` element matches the AC in `4-1-implement-dynamic-difficulty-adjustment.md`.
...
- ✓ Interfaces/API contracts extracted if applicable
  - **Evidence:** One `<interface>` entry under `<interfaces>` is present.
