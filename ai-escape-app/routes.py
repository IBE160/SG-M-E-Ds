...
from services.game_logic import create_game_session, get_game_session, update_game_session, update_player_inventory
...
def start_game():
    ...
    new_session = create_game_session(current_app.session, player_id, theme, location, difficulty)
    return jsonify({
        "id": new_session.id,
...
def move_player(session_id):
    ...
    updated_session = update_game_session(current_app.session, session_id, current_room=new_room)
    if not updated_session:
        return jsonify({"error": "Game session not found"}), 404
    return jsonify({
        "id": updated_session.id,
        "current_room": updated_session.current_room
    })
...
def handle_inventory(session_id):
    ...
    updated_session = update_player_inventory(current_app.session, session_id, item, action)
    if not updated_session:
        return jsonify({"error": "Game session not found"}), 404
    return jsonify({
        "id": updated_session.id,
