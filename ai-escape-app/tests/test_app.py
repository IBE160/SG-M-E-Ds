...
def client():
    ...
    app = create_app(config_object=type('TestConfig', (object,), {
        'TESTING': True,
...
def test_move_player(client):
    ...
    response = client.post(f'/game_session/{session_id}/move', json={'new_room': 'middle_room'})
    assert response.status_code == 200
    data = json.loads(response.data)
    assert data['id'] == session_id
...
def test_handle_inventory_add(client):
    # First create a session
    start_response = client.post('/start_game', json={'player_id': 'test_player_inventory_add'})
    session_id = json.loads(start_response.data)['id']

    response = client.post(f'/game_session/{session_id}/inventory', json={'item': 'key', 'action': 'add'})
    assert response.status_code == 200
    data = json.loads(response.data)
    assert data['id'] == session_id
...
def test_handle_inventory_remove(client):
    ...
    start_response = client.post('/start_game', json={'player_id': 'test_player_inventory_remove'})
    session_id = json.loads(start_response.data)['id']
    client.post(f'/game_session/{session_id}/inventory', json={'item': 'key', 'action': 'add'})

    response = client.post(f'/game_session/{session_id}/inventory', json={'item': 'key', 'action': 'remove'})
    assert response.status_code == 200
    data = json.loads(response.data)
    assert data['id'] == session_id
    ...
    # Test with non-existent session
    response = client.post('/game_session/9999/inventory', json={'item': 'fake_item', 'action': 'add'})
    assert response.status_code == 404
